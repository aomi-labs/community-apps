...
The backend writes
`apps/<installation-id>/cecilia-test-2/.aomi/deployment.json` and pushes a
candidate branch. This repo's release-builder CI validates that backend
manifest, builds the cdylib, and uploads release artifacts tagged
`apps-<installation-id>-cecilia-test-2-<short-source-commit>`.
