...
Starter tools plus safe crypto helpers:

| Tool | Args | Returns |
|---|---|---|
| `echo` | `message: string` | the same message |
| `greet` | `name: string`, `excited?: bool` | a greeting |
| `market_price` | `symbol: string` | current Binance spot price |
| `prepare_swap` | `from_asset`, `to_asset`, `amount`, `slippage_percent?` | validated, non-executing swap intent |

Optional Binance environment/vault variables are declared by the app: `BINANCE_API_KEY`,
`BINANCE_API_SECRET`, and `BINANCE_BASE_URL`. The market price tool only needs the base URL;
it defaults to `https://api.binance.com`. No private key or trading order is handled by this starter app.
