...
mod schema_guard {
    ...
    #[test]
    fn no_arg_tools_keep_a_property_across_provider_serialization() {
        for name in ["get_risk_snapshot", "get_credit_balance"] {
            ...
            assert!(
                s["properties"].get("reserved").is_some(),
                "{name}: the provider boundary drops empty maps, so no-arg schemas need a property"
            );

            let serialized = serde_json::to_value(s).expect("schema serializes");
            assert!(
                serialized["properties"].get("reserved").is_some(),
                "{name}: schema anchor must survive serialization"
            );
        }
    }
