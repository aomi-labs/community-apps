...
/// Args type for every tool that takes no input. schemars derives a bare
/// `{"type": "object"}` for an empty struct, and strict model providers reject
/// an object parameter schema without a `properties` key — so spell out
/// `properties: {}` (plus `required: []` / `additionalProperties: false`)
/// explicitly. Use this type for any new no-arg tool instead of a fresh empty
/// struct; the schema guard test in lib.rs enforces the invariant.
#[derive(Debug, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
#[schemars(extend("properties" = {}, "required" = [], "additionalProperties" = false))]
